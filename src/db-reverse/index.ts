...
import { createDriver, createModelFromDatabase } from "./generation/engine";
import * as TomgUtils from "./misc/utils";
import IConnectionOptions, {
  getDefaultConnectionOptions,
} from "./options/connection-options.interface";
...
function checkYargsParameters(options: DBReverseOptions): DBReverseOptions {
  const { argv } = Yargs.usage(
    ...
    secureResolvers: {
      boolean: true,
      default: options.generationOptions.secureResolvers,
      describe: "Add security to Resolvers with @Secure decorator",
    },
  });
  ...
  options.generationOptions.exportType = argv.defaultExport
    ? "default"
    : "named";
  options.generationOptions.secureResolvers = argv.secureResolvers;
  return options;
}
...
export async function useInquirer(
  ...
  if (customizeGeneration) {
      ...
      await inquirer.prompt([
        {
          choices: [
            ...
            },
            {
              ...
              checked: !!options.generationOptions.customNamingStrategyPath,
            },
            {
    ...
    if (customizations.includes("namingConvention")) {
      ...
      options.generationOptions.convertCaseFile = namingConventions.fileCase;
      options.generationOptions.convertCaseProperty =
