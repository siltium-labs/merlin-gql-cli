import { propertyIsDecoratedWithField, propertyIsSortIgnored } from "../../utils/metadata-storage";
...
export const SortTemplate = (
  ...
  return `
        ...
        ${entity.relations.filter(r => propertyIsDecoratedWithField(r.fieldName, entity.tscName) && !propertyIsSortIgnored(r.fieldName, entity.tscName)).map(r => r.relatedTable).map(fileImport => ImportsTemplate(fileImport, generationOptions)).join("\n")}

        @InputType()
        export ${defaultExport(generationOptions)} class ${sortsName} extends BaseSortFields {
          
          ${entity.columns.filter(c => propertyIsDecoratedWithField(c.tscName, entity.tscName) && !propertyIsSortIgnored(c.tscName, entity.tscName)).map(c => ColumnTemplate(c, generationOptions)).join("\n")}
          
          ${entity.relations.filter(c => propertyIsDecoratedWithField(c.fieldName, entity.tscName) && !propertyIsSortIgnored(c.fieldName, entity.tscName)).map(r => RelationTemplate(r, generationOptions)).join("\n")}
        }
      `
}
